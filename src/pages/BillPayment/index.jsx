  ...
  const handleDelete = async (item) => {
    ...
    try {
      const res = await fetch("/api/deleteCreditCard", {
        ...
        body: JSON.stringify({ sheetIndex: item.sheetIndex }),
      });
          ...
          <table className="min-w-full bg-white rounded-lg shadow-sm">
            <thead>
              <tr className="bg-gray-200">
                ...
                <th className="py-3 px-4 text-left font-semibold text-gray-600">Card Type</th>
                <th className="py-3 px-4 text-left font-semibold text-gray-600">Type</th>
                <th className="py-3 px-4 text-left font-semibold text-gray-600">Limit</th>
                <th className="py-3 px-4 text-left font-semibold text-gray-600">Remaining Limit</th>
                <th className="py-3 px-4 text-left font-semibold text-gray-600">Action</th>
              </tr>
            </thead>
            <tbody>
              {data.map((card) => (
                <tr key={card.id} className="border-t border-gray-200 hover:bg-gray-100">
                  <td className="py-3 px-4">{card.srNo}</td>
                  <td className="py-3 px-4">{card.name}</td>
                  <td className="py-3 px-4">{card.card_type}</td>
                  <td className="py-3 px-4">{card.type}</td>
                  <td className="py-3 px-4 font-bold text-blue-600">{formatINR(card.limit)}</td>
                  <td className="py-3 px-4 font-bold text-green-600">{formatINR(card.remainingLimit)}</td>
                  <td className="py-3 px-4">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        disabled={isDeleting && itemToDelete?.id === card.id}
                        className="text-red-600 hover:text-red-800 transition-colors"
                      >
                        {isDeleting && itemToDelete?.id === card.id ? (
                          <LoadingSpinner1 />
                        ) : (
                          <TrashIcon />
                        )}
